...
def cert_createTar(certid, directory):
    ...
    certtar.add(join(app.config['DIRECTORY'], 'freifunk_%s.key' % certid), ('TUN_B_%s.key' % certid))
    certtar.add(join(app.config['DIRECTORY'], 'freifunk_%s.crt' % certid), ('TUN_B_%s.crt' % certid))
